...
默认逐镜**并发**生成,每镜各自独立(快,但镜头之间画面会跳)。开「**链式衔接**」后改为**串行**:第 1 镜用自己的关键帧当首帧,此后**每镜用上一镜成片的末帧当首帧**,让镜头之间画面自然延续——适合做"一镜到底/连贯运镜"的连续感短片。

- 调接口:生成片段时传 `{"chained": true}`(`POST /api/drama/projects/{pid}/clips`),或一键全自动时传 `{"chained": true}`。
- 代价:必须串行,整条链较慢;**抽末帧需要 ffmpeg**(没装则自动回退为各镜用自己的关键帧,不中断)。
- 某镜生成失败会"断链",下一镜自动从自己的关键帧重新起头。
- 实现:`assemble.extract_last_frame`(ffmpeg 抽末帧)+ `drama.gen_clips_chained`(串行续接)。
